...
1/ Hãy tính trung bình của những con số chia hết cho 5 trong chuỗi số này
...
2/ Trích xuất những 12 con số cuối của dãy số và tính lũy thừa 2 của lần lượt các con số này
...
```{r, echo = FALSE}
round(((b[(length(b)-11) : length(b)]^2) / mean(b[(b %% 5) == 0])),
      digits = 4) -> yes
yes
```
...
**Quý Anh Chị viết code R để thực hiện các câu hỏi như trong bài tập. Em đã để sẵn phần đáp án, còn phần code R thì Anh Chị liên hệ em qua email `tuhocr@gmail.com` để nhận file đáp án nhé. Trân trọng.**
